import { GetCurrentOrder } from "@/app/services/apiOrders";
...
export const getLoggedinConfirmation = (state: RootState) => state.login.IsLoggedIn;

export const getCurrentOrderListener = (startAppListening: AppStartListening) => {
    startAppListening({
        ...
        effect: async (action, listenerApi) => {
            ...
            if(response instanceof AxiosError){
                ...
                if(response.order !== undefined){
                    ...
                    const orderWithoutItems: OrderResponse = {OrderId: currentOrder.OrderId, CardRequest: currentOrder.CardRequest, AddressRequest: currentOrder.AddressRequest};
                    const currentItems: ProductForLineItem[] = currentOrder.Items.map((item) => {
                        return {
                            id: item.productId,
                            name: item.name,
