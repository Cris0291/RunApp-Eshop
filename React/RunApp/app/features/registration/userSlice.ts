...
import { AddressResponse, AddressSettingsForm, PaymentResponse, PaymentSettingsForm } from '../payment/checkout/contracts'

const initData = localStorage.getItem("userSession");
const userSession : UserSession = initData !== null ? JSON.parse(initData) : null;
...
export const userSlice = createSlice({
    ...
    reducers: {
        ...
        addUserAddress: (state, action: PayloadAction<AddressSettingsForm | AddressResponse>) => {
            state.address = action.payload;
        },
        addUserCard: (state, action: PayloadAction<PaymentSettingsForm | PaymentResponse>) => {
...
export const {setUser, updateUser, addUserAddress, addUserCard, clearUser} = userSlice.actions

export default userSlice.reducer

export const getUserConfirmationOfRegistration = (state : RootState) => state.user.name !== "" && localStorage.getItem("tokenModel") !== null;
